...
static void BENCHMARK_boost_lockfree(benchmark::State &state)
{
    constexpr auto iters = 100'000'000l;
    constexpr auto fifoSize = 65536;
    boost::lockfree::spsc_queue<std::int_fast64_t, boost::lockfree::fixed_sized<true>> fifo{fifoSize};
    ...
    auto t = std::jthread([&] {
      ...
      for (std::int_fast64_t i = 0; i < fifoSize; ++i) {
        ...
        while (auto again = not fifo.pop(val)) benchmark::DoNotOptimize(again);
      }
...
BENCHMARK(BENCHMARK_boost_lockfree);
BENCHMARK_MAIN();
